...
assert isinstance(response_json["data"][0]["breed"],
                  str), f"Параметр 'breed' неверный тип данных"
assert isinstance(response_json["data"][0]["country"],
                  str), f"Параметр 'country' неверный тип данных"
assert isinstance(response_json["data"][0]["origin"],
                  str), f"Параметр 'origin' неверный тип данных"
assert isinstance(response_json["data"][0]["coat"],
                  str), f"Параметр 'coat' неверный тип данных"
assert isinstance(response_json["data"][0]["pattern"],
                  str), f"Параметр 'pattern' неверный тип данных"
assert isinstance(response_json["current_page"], int,) f"Параметр 'current_page' неверный тип данных"
assert isinstance(response_json["from"], int,) f"Параметр 'from' неверный тип данных"
assert isinstance(response_json["last_page"], int,) f"Параметр 'last_page' неверный тип данных"
assert isinstance(response_json["per_page"], int,) f"Параметр 'per_page' неверный тип данных"
assert isinstance(response_json["to"], int,) f"Параметр 'to' неверный тип данных"
assert isinstance(response_json["total"], int,) f"Параметр 'total' неверный тип данных"
assert response_json["per_page"] == params.get(
    "limit"), f"Параметр 'per_page' не равен значению 'limit'"

assert response_json["first_page_url"], f"Параметра 'first_page_url' нет в ответе"
assert validators.url(
    response_json["first_page_url"]), f"Не валидный формат 'first_page_url'"
assert response_json["last_page_url"], f"Параметра 'last_page_url' нет в ответе"
assert validators.url(
    response_json["last_page_url"]), f"Не валидный формат 'last_page_url'"
